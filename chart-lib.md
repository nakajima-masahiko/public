...
先頭 2 枚のカードだけがチラついていた主因は、**`ResizeObserver.observe()` の初期通知タイミング**にあります。

デモのチャート生成ループは同期的に実行されるため、chart0・chart1 の構築時点ではまだ
全 9 枚のカードが DOM に追加されておらず、レイアウトが最終状態になっていません。
その後 ResizeObserver の初期通知がブラウザから非同期で届いた時点では全カードが揃い、
先頭チャートのコンテナサイズがコンストラクタ時の計測値とズレていることがあります。
このズレが "サイズ変化" と誤検知され、`renderer.resize()` → `_draw()` が余計に実行されて
キャンバスが一瞬クリアされることでチラつきが発生していました。
後続チャートは構築時点で最終レイアウトが確定しているためズレが生じず、スキップされます。
